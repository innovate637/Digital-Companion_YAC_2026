...
def load_llm():
    return init_chat_model(
        ...
        model_kwargs={
            ...
            # GEN_TEMPERATURE / GEN_MAX_TOKENS) — only these two were set there, so only
            # these two are set here. The previous top_p/top_k/presence_penalty/
            # frequency_penalty overrides were never evaluated and have been removed.
            "temperature": 0.3,
            "max_tokens": 400,
        },
    )
...
if prompt := st.chat_input("What is on your mind?"):
    ...
    # Gemma's chat template enforces strict user/assistant alternation *starting with a
    # user turn* (after the single leading system message). Its jinja raises
    #   "Conversation roles must alternate user/assistant/user/assistant/..."
    # when (message['role'] == 'user') != (loop.index0 % 2 == 0), and a template render
    # failure surfaces to the client as an HTTP 500 InternalServerError.
    #
    # The opening greeting is an assistant message with no user turn before it, so it sat
    # at index 0 and tripped that check on EVERY request - a deterministic 500, not the
    # intermittent server flakiness it first looked like. It is display-only: keep it in
    # session_state for the UI, but never send it to the model.
    history = list(st.session_state.messages)
    while history and history[0].get("role") != "user":
        history.pop(0)

    # convert session messages to langchain message objects, collapsing any consecutive
    # same-role turns (which would break the same alternation rule).
    session_messages = []
    ...
    for m in history:
        ...
        if session_messages and type(session_messages[-1]) is type(message):
            session_messages[-1] = type(message)(
    ...
    if response is None:
        assistant_text = "Sorry, I'm having trouble connecting right now. Please try again in a moment."
    else:
        try:
